...
def after_build(source, target, env):
    ...
    for f in glob ('bin/GW_Z*.bin'):
        os.unlink (f)

    exit_code = call("python2 tools/merge_bin_esp.py --output_folder ./bin --output_name GW_Z.bin --bin_path bin/bootloader_dio_40m.bin bin/firmware.bin bin/partitions.bin --bin_address 0x1000 0x10000 0x8000", shell=True)
    ...
    NEW_NAME_FULL = 'bin/GW_Z_v'+VERSION_NUMBER+'.bin'
    #NEW_NAME = 'bin/ZigStarGW.bin'

    shutil.move('bin/firmware.bin', NEW_NAME_FULL)
