...
//const char* deviceModel = "GW_Z_LUW02P";
const char* deviceModel = modelID;
void mDNS_start();
void connectWifi();
void handlelongBtn();
...
bool loadConfigEther(){
  ...
  if (!configFile){
    ...
    doc["mask"] = "";
    doc["gw"] = "";
    doc["disablePingCtrl"] = 1;
    String StringConfig = "{\"dhcp\":1,\"ip\":\"\",\"mask\":\"\",\"gw\":\"\",\"disablePingCtrl\":1}";
...
{ String temphost=String("_") + String(modelID);
  const char* host = const_cast<char *>(temphost.c_str());// PR#84111
...
void setup(){
  ...
  //powersupply
  pinMode(PowerPOE,INPUT);
  pinMode(PowerUSB,INPUT);
