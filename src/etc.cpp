...
bool hasPOE()
{
  if (modelID=="GW_Z_LUW02P" || modelID=="GW_Z_LUW03P")
  {
    return true;
  }
  else
...
void adapterModeUSB()
{
  printLogMsg("Switched GW to USB mode");
  DEBUG_PRINTLN(F("Switched GW to USB mode"));
...
void adapterModeLAN()
{
  printLogMsg("Switched GW to LAN mode");
  DEBUG_PRINTLN(F("Switched GW to LAN mode"));
