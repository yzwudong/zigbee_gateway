...
const char *contTypePng = "img/png";
const char *contTypeGif = "img/gif";
const char *contTypeWoff = "font/woff";
...
void initWebServer() {
    ...
    //serverWeb.on("/js/toast.js", []() { sendGzip(contTypeTextJs, toast_js_gz, toast_js_gz_len); });
    serverWeb.on("/fonts/glyphicons.woff",[]() { sendGzip(contTypeWoff, glyphicons_woff_gz, glyphicons_woff_gz_len); });
    serverWeb.on("/img/logo.png", []() { sendGzip(contTypePng, logo_png_gz, logo_png_gz_len); });
    serverWeb.on("/img/wait.gif", []() { sendGzip(contTypeGif, wait_gif_gz, wait_gif_gz_len); });
    serverWeb.on("/img/nok.png", []() { sendGzip(contTypePng, nok_png_gz, nok_png_gz_len); });
    serverWeb.on("/img/ok.png", []() { sendGzip(contTypePng, ok_png_gz, ok_png_gz_len); });
...
void handleRoot() {
        ...
        if (ConfigSettings.apStarted) {  // AP active
            ...
            doc[wifiIp] = "192.168.1.1 (web interface)";
        ...
        //if (!ConfigSettings.hasPOE)
        if( !hasPOE())
        {
            doc["powerSupply"] = "USB";
        }else
        {
            // if (ConfigSettings.powerPOE)
            if(digitalRead(PowerPOE))
            {
                doc["powerSupply"] = "POE";
            }
            //if (ConfigSettings.powerUSB)
            if(digitalRead(PowerUSB))
            {
                doc["powerSupply"] = " USB";
            }
            if (digitalRead(PowerPOE) && digitalRead(PowerUSB))
            {
                doc["powerSupply"] = "! POE AND USB";
            }
            //DEBUG_PRINTLN("powerPOE="+ConfigSettings.powerPOE);
            //DEBUG_PRINTLN("powerUSB="+ConfigSettings.powerUSB);
        }
